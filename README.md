...
你可能会问：截图、AI 对话、手机控制、桌面自动化这么多事情，是靠什么让程序不会卡的？

### 打个比方：餐厅厨房

| 角色 | 对应代码 | 干什么 |
|------|----------|--------|
| **店长（主线程）** | Qt QEventLoop | 接待客人、收银、管全局。所有事情最终都要经过店长。 |
| **煲汤师傅（对话线程）** | StreamWorker | 把汤料放火上就可以去干别的，不用守着——汤好了自然会叫。这就是"协程"。 |
| **外卖小哥（子进程）** | run_gui_agent.py | 出去送餐，完全独立行动。店长给他一张纸条，他送完回来交差。 |
| **前台接线员（远程线程）** | RemoteServer | 接电话、记订单，把纸条递给店长。 |
| **清洁阿姨（记忆线程）** | MemWorker | 等客人走了才出来收拾，偶尔干活。 |

### 他们怎么配合

```
你点了一份"AI 煲汤" (Ctrl+D 截图提问)

店长（主线程）→ 把汤料交给煲汤师傅（对话线程）
煲汤师傅 → 把汤放火上（等豆包返回），自己去看手机了（协程让出）
煲汤师傅 → 汤滚了（收到一个 token）→ 喊店长："加进去！"（信号通知）
店长 → 把 token 写到悬浮窗上（更新 UI）
煲汤师傅 → 继续等下一个 token → 喊 → 继续等... 直到汤煲完

整个过程店长始终在接待、收银、处理热键，
没有被煲汤卡住——因为煲汤师傅在另一个房间干活。
```
...
| 谁 | 在哪干 | 干什么 | 怎么告诉店长结果 |
|----|--------|--------|-----------------|
| **店长（主线程）** | 自己脑子里 | 管界面、热键、每秒钟检查一次手机有没有新订单 | 他说了算 |
| **煲汤师傅（对话线程）** | 隔壁厨房 | AI 对话流式输出，一个字一个字往外蹦 | 每蹦一个字就喊一声（Qt 信号）|
| **外卖小哥（子进程）** | 完全独立的店外 | 控制鼠标自动操作桌面，截图→AI决策→点击→循环 | 通过纸条（进度打印 + 结果文件）交给店长 |
| **前台接线员（远程线程）** | 电话旁 | 手机发来的指令、手机要的截图 | 把订单写在小黑板上（变量），店长定时去看 |
| **清洁阿姨（记忆线程）** | 客人走后 | 从对话里提取重要信息（长期记忆） | 干完了汇报一声 |

### 为什么要分开

- **店长不能自己去煲汤** — 得一直守在门口（UI 不能冻结）
- **外卖小哥必须独立** — 他骑的车和店长的收银机用的是同一套控制系统，放一起会打架（两个事件循环冲突）
- **煲汤师傅用协程不用多个人** — 他同时煲 3 锅汤只需要一个人（单线程管多协程），因为大部分时间在等汤滚（等网络），不是真的在忙
- **店长和师傅之间的喊话** — Qt 信号槽机制：师傅喊一声，Qt 自动把话抄下来放到店长的待办清单里，店长有空了再看
